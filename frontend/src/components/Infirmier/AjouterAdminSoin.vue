    ...
    <div v-if="selectedSoin" class="administration-form-container">
      ...
      <div class="soin-info-panel">
        <div class="info-group">
          <h4>Patient</h4>
          <p>
            {{ selectedSoin.nom_patient }} {{ selectedSoin.prenom_patient }}
          </p>
        </div>
      ...
      <div
        v-if="selectedSoin.medicaments && selectedSoin.medicaments.length > 0"
        class="medicaments-panel"
      >
        <h4>Médicaments à administrer</h4>
        <ul class="medicaments-list">
          <li
            v-for="(med, index) in selectedSoin.medicaments"
            :key="index"
      ...
      <form @submit.prevent="submitForm" class="admin-form">
        ...
        <div class="form-actions">
          <button type="button" class="btn-cancel" @click="deselectSoin">
            Annuler
          </button>
          <button
            type="submit"
            class="btn-save"
            :disabled="!formData.id_infirmier"
          >
...
export default {
  ...
  methods: {
        ...
        const enrichedSoins = await Promise.all(
          soinsData.map(async (soin) => {
            try {
              ...
              if (
                soinDetail.administrations &&
                soinDetail.administrations.length > 0
              ) {
                // Trier les administrations par date (la plus récente en premier)
                const sortedAdmins = [...soinDetail.administrations].sort(
                  (a, b) => {
    ...
    async submitForm() {
      ...
      try {
        console.log(
          "Envoi des données pour l'administration du soin:",
          this.formData
        );
