    ...
    <form @submit.prevent="submitForm" class="reunion-form">
      ...
      <div class="form-section">
        ...
        <div
          class="form-message"
          v-if="
            formData.medecinIds.length === 0 &&
            formData.infirmierIds.length === 0
          "
        >
  ...
  computed: {
    ...
    isFormValid() {
      return (
        ...
        this.formData.sujet &&
        (this.formData.medecinIds.length > 0 ||
          this.formData.infirmierIds.length > 0)
      );
    },
  },
  mounted() {
    ...
    async submitForm() {
      ...
      try {
        ...
        const dateReunion = new Date(
          `${this.formData.dateReunionDate}T${this.formData.dateReunionTime}`
        );
